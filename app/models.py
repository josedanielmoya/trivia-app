...
class User(UserMixin, db.Model):
    ...
    def get_total_games(self):
        return self.hosted_games.count() + self.joined_games.count()
...
class Game(db.Model):
    ...
    time_per_q = db.Column(db.Integer, nullable=False, default=30)
    status = db.Column(db.String(10), nullable=False, default="waiting")
    questions_json = db.Column(db.Text, nullable=True)
    winner_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))

    players = db.relationship('User', secondary=game_players, backref=db.backref('joined_games', lazy='dynamic'))
...
class Answer(db.Model):
    ...
    given_answer = db.Column(db.String(256), nullable=True)
